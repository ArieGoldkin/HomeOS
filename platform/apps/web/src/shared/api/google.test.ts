...
describe("startGoogleConnect (#231 — session-gated)", () => {
  it("returns { url } on 200, riding the session cookie (credentials: include), no bearer", async () => {
    let credentials: RequestCredentials | undefined;
    let authHeader: string | null = null;
    server.use(
  ...
  it("maps 401 to the 'auth' reason (no session)", async () => {
    server.use(
      http.get("*/oauth/google/connect-url", () => new HttpResponse(null, { status: 401 })),
    );
    await expect(startGoogleConnect()).rejects.toMatchObject({ reason: "auth" });
  });

  it("maps 403 to the 'auth' reason, distinct from 429/503", async () => {
    server.use(
  ...
  it("maps 429 to the distinct 'rate_limited' reason", async () => {
    ...
    await expect(startGoogleConnect()).rejects.toMatchObject({ reason: "rate_limited" });
  });
  ...
  it("maps 503 to the distinct 'not_configured' reason (dark)", async () => {
    ...
    await expect(startGoogleConnect()).rejects.toMatchObject({ reason: "not_configured" });
  });
  ...
  it("throws a GoogleConnectError instance with the status preserved", async () => {
    ...
    const err = await startGoogleConnect().catch((e) => e);
...
describe("disconnectGoogle (#231 — session-gated)", () => {
  it("resolves on a 204, riding the session cookie (credentials: include), no bearer", async () => {
    let credentials: RequestCredentials | undefined;
    let authHeader: string | null = null;
    server.use(
  ...
  it("throws a GoogleConnectError on a non-2xx (e.g. 401)", async () => {
    ...
    await expect(disconnectGoogle()).rejects.toBeInstanceOf(GoogleConnectError);
  });
});
